...
export const apiSlice = createApi({
	...
	baseQuery: async (args, api, extraOptions) => {
		...
		if (response.error && response.error.status && response.error.status === 401 && response.error.data && response.error.data.message && response.error.data.message === 'authentication_notAuthenticated') {
